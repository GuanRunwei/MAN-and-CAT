...
if __name__ == "__main__":
    ...
    # 'model_data/yolov4_tiny_weights_coco.pth'
    # ------------------------------------------------------#
    ...
    #   weight_decay    权值衰减，可防止过拟合 默认5e-4
    # ------------------------------------------------------------------#
    optimizer_type = "sgd"
    momentum = 0.937
    weight_decay = 5e-5
    ...
    #   创建yolo模型
    # ------------------------------------------------------#
    model = YoloBody(anchors_mask, num_classes, pretrained=pretrained, phi=phi, input_shape=input_shape)
    if not pretrained:
        weights_init(model)
